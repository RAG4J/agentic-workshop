...
@Agent(name = "SciFiAgent",
        description = "An agent specialized in science fiction topics that can answer questions about SciFi and write short stories about science fiction characters.",
        version = "1.0.0")
public record SciFiAgent() {
    private static final Logger logger = LoggerFactory.getLogger(SciFiAgent.class);

    @AchievesGoal(
            description = "Answers questions about science fiction or writes short stories about SciFi characters based on user requests."
    )
    @Action
